...
function SearchPageContent() {
  ...
  // Track whether user has manually changed sort to prevent auto-override
  const userChangedSortRef = useRef(false);

  useEffect(() => {
    // Only auto-switch sort on initial load or when query changes
    // Skip if user has manually changed the sort
    ...
    if (!urlSort) {
      const newSort: SortType = debouncedQuery.trim()
        ? "relevance"
        : "downloads";
      if (sort !== newSort) {
        setSort(newSort);
  ...
  const availableSubtypes = useMemo<readonly Subtype[]>(
    () =>
      selectedFormat
  ...
  useEffect(() => {
    const fetchStarredIds = async () => {
      ...
      try {
        ...
        const localCollections = localStorage.getItem(
          "prpm_starred_collections",
        );
      ...
      if (token) {
        try {
          ...
          collectionsData.collections.forEach((coll) =>
            ids.add(`${coll.scope}/${coll.name_slug}`),
          );
        } catch (err) {
  ...
  const fetchPackages = async () => {
    ...
    try {
      ...
      if (starredOnly) {
        ...
        if (selectedFormat) {
          filteredPackages = filteredPackages.filter(
            (pkg) => pkg.format === selectedFormat,
          );
        }
        if (selectedSubtype) {
          filteredPackages = filteredPackages.filter(
            (pkg) => (pkg as any).subtype === selectedSubtype,
          );
        }
        if (selectedCategory) {
          filteredPackages = filteredPackages.filter(
            (pkg) => (pkg as any).category === selectedCategory,
          );
        }
        if (selectedLanguage) {
          filteredPackages = filteredPackages.filter(
            (pkg) => (pkg as any).language === selectedLanguage,
          );
        }
        if (selectedFramework) {
          filteredPackages = filteredPackages.filter(
            (pkg) => (pkg as any).framework === selectedFramework,
          );
        }
        ...
        if (selectedAuthor) {
          filteredPackages = filteredPackages.filter((pkg) =>
            pkg.name.startsWith(`@${selectedAuthor}/`),
          );
        }
        ...
        switch (sort) {
          case "downloads":
            filteredPackages.sort(
              (a, b) =>
                ((b as any).total_downloads || 0) -
                ((a as any).total_downloads || 0),
            );
            break;
          case "created":
            filteredPackages.sort(
          ...
          case "quality":
            filteredPackages.sort(
              (a, b) =>
                ((b as any).quality_score || 0) -
                ((a as any).quality_score || 0),
            );
            break;
          case "rating":
            filteredPackages.sort(
              (a, b) =>
                ((b as any).rating_average || 0) -
                ((a as any).rating_average || 0),
            );
            break;
          case "relevance":
          default:
  ...
  const fetchCollections = async () => {
    ...
    try {
      ...
      if (starredOnly) {
        ...
        if (selectedCategory) {
          filteredCollections = filteredCollections.filter(
            (coll) => (coll as any).category === selectedCategory,
          );
        }
        ...
        switch (sort) {
          case "downloads":
            filteredCollections.sort(
              (a, b) =>
                ((b as any).downloads || 0) - ((a as any).downloads || 0),
            );
            break;
          case "created":
            filteredCollections.sort(
  ...
  const clearFilters = () => {
    ...
    setTagInput("");
    setShowTagSuggestions(false);
    // Reset user sort preference so auto-sort can work again
                  ...
                  <div className="mb-6">
                    ...
                    {selectedFormat === "agents.md" && (
                      <div className="mt-3 p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                        ...
                        <p className="text-xs text-gray-400">
                          OpenAI Codex • GitHub Copilot • Google Gemini • Any
                          tool supporting the open standard
                        </p>
                      </div>
                    )}
                    ...
                    {selectedFormat === "gemini" && (
                      <div className="mt-3 p-3 bg-gray-500/10 border border-gray-500/30 rounded-lg">
                        <p className="text-xs text-gray-400">
                          Tool-specific format for <strong>Gemini CLI</strong>{" "}
          ...
          <div className="lg:col-span-3">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm text-blue-200 font-medium mb-1">
                      ...
                      {fallbackOriginalQuery &&
                        ` for "${fallbackOriginalQuery}"`}
                    </p>
                    ...
                    <p className="text-xs text-blue-300/60 flex items-center gap-1">
                      ...
                      PRPM converts all formats for you — install any package
                      with{" "}
                      <code className="px-1 py-0.5 bg-blue-500/20 rounded">
                        --as {selectedFormat || "any-format"}
                      </code>
                    </p>
                  </div>
                </div>
            ...
            {loading ? (
              ...
              <>
                            ...
                            {packages.map((pkg) => (
                              ...
                              >
                                  ...
                                  <div className="flex items-start justify-between mb-2">
                                    <div className="flex-1">
                                      <div className="flex items-center gap-2 mb-2">
                                        ...
                                        {pkg.verified && (
                                          <svg
                                            className="w-5 h-5 text-prpm-accent"
                                            fill="currentColor"
                                            viewBox="0 0 20 20"
                                          >
                                            <path
                                  ...
                                  <div className="mt-4 pt-4 border-t border-prpm-border space-y-3">
                                    ...
                                    <div className="space-y-2">
                                      <div className="flex items-center justify-between gap-2">
                                        ...
                                        >
                                          {copiedId === pkg.id ? (
                                            ...
                                            <svg
                                              className="w-4 h-4 text-gray-400 hover:text-prpm-accent"
                                              fill="none"
                                              stroke="currentColor"
                                              viewBox="0 0 24 24"
                                            >
                                      ...
                                      {selectedFormat &&
                                        pkg.format !== selectedFormat && (
                                          <p className="text-xs text-gray-400">
                                            💡 PRPM converts all formats for you
                                            — this {pkg.format} package will
                                            work with {selectedFormat}
                                          </p>
                                        )}
                                    </div>
                                ...
                                <button
                                  ...
                                  title={
                                    starredIds.has(pkg.id) ? "Unstar" : "Star"
                                  }
                                >
                                  <svg
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-400 mb-2">
                            Collections are curated sets of packages grouped
                            together for a specific purpose or workflow. Install
                            an entire collection at once to get everything you
                            need.
                          </p>
                          <a
                      ...
                      collections.map((collection) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            title={
                              starredIds.has(
                                `${collection.scope}/${collection.name_slug}`,
                              )
                                ? "Unstar"
                                : "Star"
                            }
                ...
                {/* Pagination */}
                {total > limit &&
                  (packages.length > 0 || collections.length > 0) && (
