    ...
    <main className="min-h-screen bg-prpm-dark">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3">
            {/* Fallback message */}
            {isFallbackResult && fallbackOriginalQuery && (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm text-yellow-200 font-medium mb-1">
                      No results found for "{fallbackOriginalQuery}"
                    </p>
                    <p className="text-sm text-yellow-300/80">
                      Showing top 10 most popular packages
                      {selectedSubtype && ` (${selectedSubtype}`}
                      {selectedSubtype && selectedFormat && ` for ${selectedFormat}`}
                      {selectedSubtype && ')'}
                      {!selectedSubtype && selectedFormat && ` (${selectedFormat} format)`}
                      {' instead'}
                    </p>
                  </div>
                </div>
