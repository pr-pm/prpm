/**
 * Trae Parser
 * Converts Trae rule files to canonical format
 *
 * Trae uses markdown files in .trae/rules/ directory
 * Files are simple markdown with project-specific rules and conventions
 */
...
/**
 * Parse Trae rule file to canonical format
 */
export function fromTrae(
  content: string,
  metadata: Partial<PackageMetadata> & Pick<PackageMetadata, 'id' | 'name' | 'version' | 'author'>
): CanonicalPackage {
  // Parse markdown body (no frontmatter in Trae)
  ...
  const pkg: Partial<CanonicalPackage> = {
    ...
    tags: metadata.tags || ['trae', ...inferTags(content)],
  ...
  // Trae files are rules by default
