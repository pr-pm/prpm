/**
 * Zencoder Parser
 * Converts Zencoder rule files to canonical format
 *
 * Zencoder uses markdown files in .zencoder/rules/ directory
 * Files support optional YAML frontmatter with: description, globs, alwaysApply
 */
...
/**
 * Parse Zencoder rule file to canonical format
 */
export function fromZencoder(
  content: string,
  metadata: Partial<PackageMetadata> & Pick<PackageMetadata, 'id' | 'name' | 'version' | 'author'>
): CanonicalPackage {
  // Parse frontmatter if exists (optional for Zencoder)
  ...
  const description = frontmatter.description || metadata.description || extractDescription(body);
  ...
  const pkg: Partial<CanonicalPackage> = {
    ...
    tags: metadata.tags || ['zencoder', ...inferTags(body)],
  ...
  // Zencoder files are rules by default
...
/**
 * Parse YAML frontmatter from Zencoder file (optional)
 */
    ...
    if (parsed && typeof parsed === 'object') {
      ...
      if ('globs' in parsed) {
        if (Array.isArray(parsed.globs)) {
          frontmatter.globs = parsed.globs.filter((g): g is string => typeof g === 'string');
        } else if (typeof parsed.globs === 'string') {
