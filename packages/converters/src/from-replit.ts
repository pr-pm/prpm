/**
 * Replit Parser
 * Converts Replit replit.md files to canonical format
 *
 * Replit uses replit.md in the project root for configuration
 * Files are simple markdown with project-specific configuration and instructions
 */
...
/**
 * Parse Replit replit.md file to canonical format
 */
export function fromReplit(
  content: string,
  metadata: Partial<PackageMetadata> & Pick<PackageMetadata, 'id' | 'name' | 'version' | 'author'>
): CanonicalPackage {
  // Parse markdown body (no frontmatter in Replit)
  ...
  const pkg: Partial<CanonicalPackage> = {
    ...
    tags: metadata.tags || ['replit', ...inferTags(content)],
  ...
  // Replit files are rules by default (project configuration)
