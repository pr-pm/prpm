...
/**
 * Tests for init --scan command (formerly catalog command)
 *
 * Note: The scanner scans from a root directory and looks for known paths
 * like .claude/skills/, .cursor/rules/, etc. relative to that root.
 * We pass "." to scan from the test directory root.
 */

import { mkdir, writeFile, rm } from "fs/promises";
import { join } from "path";
import { tmpdir } from "os";
import { initPackage } from "../commands/init";
...
describe("init --scan command (formerly catalog)", () => {
  let testDir: string;
  ...
  describe("lockfile filtering", () => {
    it("should include user-created packages not in lockfile", async () => {
      ...
      await mkdir(join(testDir, ".claude/skills/my-skill"), {
        recursive: true,
      });
      await writeFile(
      ...
      // Run init --scan from the root (.) to scan .claude/skills
      await initPackage({ scan: true, dryRun: true }, ["."]);
    ...
    it("should exclude packages that are in lockfile", async () => {
      // Create a skill
      await mkdir(join(testDir, ".claude/skills/installed-skill"), {
        recursive: true,
      });
      await writeFile(
      ...
      lockfile.packages["installed-skill"] = {
        version: "1.0.0",
        resolved:
          "https://registry.prpm.dev/packages/installed-skill/1.0.0.tgz",
      ...
      // Verify package was skipped (note: displays frontmatter name, not folder name)
      expect(consoleSpy).toHaveBeenCalledWith(
    ...
    it("should exclude packages with scoped names in lockfile", async () => {
      // Create a skill
      await mkdir(join(testDir, ".claude/skills/typescript-safety"), {
        recursive: true,
      });
      await writeFile(
      ...
      lockfile.packages["@author/typescript-safety"] = {
        version: "1.0.0",
        resolved:
          "https://registry.prpm.dev/packages/@author/typescript-safety/1.0.0.tgz",
    ...
    it("should include user packages even when other packages are in lockfile", async () => {
      ...
      await mkdir(join(testDir, ".claude/skills/my-custom-skill"), {
        recursive: true,
      });
      await writeFile(
      ...
      // Verify installed-skill was skipped (note: displays frontmatter name)
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining("Skipping Installed"),
      );
    ...
    it("should show helpful message when all packages are filtered out", async () => {
      ...
      await mkdir(join(testDir, ".claude/skills/skill-one"), {
        recursive: true,
      });
      await writeFile(
      ...
      await mkdir(join(testDir, ".claude/skills/skill-two"), {
        recursive: true,
      });
      await writeFile(
  ...
  describe("multiple formats", () => {
    it("should filter lockfile packages across multiple formats", async () => {
      ...
      await mkdir(join(testDir, ".claude/skills/claude-skill"), {
        recursive: true,
      });
      await writeFile(
      ...
      // Scan from root - will find both .claude/skills and .cursor/rules
      await initPackage({ scan: true, dryRun: true }, ["."]);
      ...
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringMatching(/my-rule/));
