...
async function handleGitHubInstall(
  ...
  try {
    ...
    // Display cache source
    const cacheLabels: Record<string, string> = {
      local: chalk.green('(local cache)'),
      registry: chalk.cyan('(registry cache)'),
      github: '',
    };
    const cacheLabel = cacheLabels[result.cacheSource] || '';
    console.log(`   ${chalk.dim(`Resolved to commit: ${result.commitSha.slice(0, 7)}`)} ${cacheLabel}`)
