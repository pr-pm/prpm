...
export async function packageRoutes(server: FastifyInstance) {
  ...
  server.get(
    ...
    async (
      request: FastifyRequest<{ Querystring: ListPackagesQuery }>,
      reply: FastifyReply,
    ) => {
      const {
        search,
      ...
      const orderByClause =
        ALLOWED_SORT_COLUMNS[sort] || ALLOWED_SORT_COLUMNS["downloads"];
      ...
      const whereClauseWithPrefix = whereClause
        .replace(/(\w+)\s*=/g, "p.$1 =")
        .replace(/(\w+)\s+ILIKE/g, "p.$1 ILIKE");
  ...
  server.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { packageName: rawPackageName } = request.params as {
        packageName: string;
      };
      ...
      if (userId) {
        ...
        server.log.debug(
          { packageName, userId },
          "Checking private package access",
        );
        ...
        server.log.debug(
          { packageName, userId, found: !!pkg },
          "Private package query result",
        );
      } else {
      ...
      if (!pkg) {
        server.log.debug(
          { packageName, userId: userId || "none" },
          "Package not found",
        );
  ...
  server.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { packageName: rawPackageName, version: versionParam } =
        request.params as { packageName: string; version: string };
      const userId = request.user?.user_id;
      ...
      if (versionParam.endsWith(".tar.gz")) {
        ...
        if (
          pkgVersion.metadata &&
          typeof pkgVersion.metadata === "object" &&
          "content" in pkgVersion.metadata
        ) {
          ...
          if (!content || content.trim() === "" || content.trim() === "---") {
            const metadata = pkgVersion.metadata as {
              sourceUrl?: string;
              content?: string;
            };
            if (metadata.sourceUrl) {
              try {
                ...
                if (response.ok) {
                  ...
                  server.log.warn(
                    `Failed to fetch content from ${rawUrl}: ${response.status}`,
                  );
                }
              } catch (error) {
                server.log.error(
                  { error, sourceUrl: metadata.sourceUrl },
          ...
          if (content && content.trim() !== "" && content.trim() !== "---") {
            ...
            reply.header(
              "Content-Disposition",
              `attachment; filename="${pkgName.replace(/[^a-z0-9-]/gi, "-")}-${version}.tar.gz"`,
            );
        ...
        if (pkgVersion.tarball_url) {
          try {
            ...
            return reply.status(404).send({
              ...
              message:
                "This package may need to be re-published. Please contact support or try again later.",
            });
          }
        }
      ...
      if (isMultipart) {
        ...
        for await (const part of parts) {
          if (part.type === "field" && part.fieldname === "manifest") {
            ...
            part.type === "field" &&
            part.fieldname === "publish_as_author"
          ) {
        ...
        if (!manifest || !tarballBase64) {
          return reply
            .status(400)
            .send({ error: "Missing manifest or tarball" });
        }
      ...
      try {
        ...
        if (!packageName || !version || !description || !format) {
          return reply.status(400).send({
            ...
            message:
              "Missing required fields: name, version, description, or format",
          });
        }
        ...
        if (orgSlug) {
          ...
          server.log.info(
            ...
            "Auto-prefixed package name with author username",
          );
        } else {
        ...
        if (!/^(@[a-z0-9-]+\/)?[a-z0-9-]+$/.test(packageName)) {
          return reply.status(400).send({
            ...
            message:
              "Package name must be lowercase alphanumeric with hyphens only",
          });
        }
        ...
        if (pkg) {
          ...
          if (pkg.author_id === userId || request.user.is_admin) {
            ...
            if (
              orgMembership &&
              ["owner", "admin", "maintainer"].includes(orgMembership.role)
            ) {
              hasPermission = true;
            }
          }
        ...
        try {
          ...
          extract.on("entry", (header: any, stream: any, next: any) => {
            ...
            // Exclude metadata files that shouldn't be part of searchable content
            const EXCLUDED_FROM_CONTENT = [
              "prpm.json",
              "package.json",
              "LICENSE",
              "LICENSE.md",
            ...
            const shouldIncludeContent =
              header.type === "file" &&
              !EXCLUDED_FROM_CONTENT.includes(fileName);
          ...
          server.log.info(
            ...
            "Extracted file metadata and content from tarball",
          );
        } catch (error) {
          server.log.warn(
            { packageName, error },
            "Failed to extract file metadata from tarball",
          );
        ...
        try {
          if (fullContent) {
            ...
            const metadata = {
              ...
              dependencies: manifest.dependencies as
                | Record<string, string>
                | undefined,
              peerDependencies: manifest.peerDependencies as
                | Record<string, string>
                | undefined,
              ...
              switch (format) {
                ...
                case "ruler":
                  {
                    ...
                    if (!result.content) {
                      throw new Error(
                        "Ruler conversion produced empty content",
                      );
                    }
                ...
                default:
                  ...
                  server.log.warn(
                    ...
                    "Unknown format, using cursor converter as fallback",
                  );
                  canonicalPkg = fromCursor(fullContent, metadata);
              }
        ...
        (async () => {
          try {
            ...
            if (packageContentForScoring) {
              ...
              const qualityScore = await updatePackageQualityScore(
                server,
                pkg.id,
                packageContentForScoring,
              );
              ...
              const needsMetadata =
                !language || !framework || !currentPkg?.category;
        ...
        if (process.env.OPENAI_API_KEY) {
          (async () => {
            try {
              ...
              if (result.success && !result.skipped) {
                server.log.info(
                  { packageId: pkg.id, packageName },
                  "Embedding generated on publish",
                );
              } else if (result.error) {
                server.log.warn(
                  { packageId: pkg.id, packageName, error: result.error },
                  "Failed to generate embedding on publish (non-blocking)",
                );
              }
            } catch (error) {
              server.log.warn(
                { packageId: pkg.id, packageName, error: String(error) },
                "Exception generating embedding on publish (non-blocking)",
              );
            }
          })();
        } else {
          server.log.debug(
            { packageId: pkg.id },
            "Skipping embedding generation - OPENAI_API_KEY not configured",
          );
        }
  ...
  server.get(
        ...
        params: {
          ...
          properties: {
            id: {
              type: "string",
              description:
                'Package ID or name (e.g., "@username/package-name")',
            },
          },
        },
        querystring: {
    ...
    async (request, reply) => {
      ...
      const { limit = 10, min_confidence = 10 } = request.query as {
        limit?: number;
  ...
  server.get(
    "/ssg-data",
    {
      schema: {
        tags: ["packages"],
        description:
          "Get all packages with full content for SSG (requires X-SSG-Token header)",
        headers: {
          type: "object",
          properties: {
    ...
    async (
      ...
      try {
        ...
        if (!expectedToken) {
          server.log.error(
            "SSG_DATA_TOKEN environment variable not configured",
          );
          return reply.code(500).send({
            error: "Internal Server Error",
        ...
        if (!ssgToken || ssgToken !== expectedToken) {
          server.log.warn(
            { ip: request.ip },
            "Unauthorized SSG data access attempt",
          );
        ...
        server.log.info(
          { format, packageName, limit, offset },
          "Fetching SSG data",
        );
  ...
  server.post(
    ...
    async (request, reply) => {
      const { package_id, version, session_id, format, install_batch_id } =
        request.body as {
          package_id: string;
          version: string;
          session_id: string;
