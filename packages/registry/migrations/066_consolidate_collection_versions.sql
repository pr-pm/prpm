...
-- First, let's see what we're dealing with (informational, wrapped in DO block)
DO $$
DECLARE
  dup_count INTEGER;
BEGIN
...
-- Step 3: Update collection_stars to point to latest version
-- For each user's star on an old version, update to point to latest version
UPDATE collection_stars cs
SET collection_id = cc.latest_id
FROM collections old_c
JOIN collection_consolidation cc ON old_c.name_slug = cc.name_slug
WHERE cs.collection_id = old_c.id
  AND old_c.id != cc.latest_id
  AND NOT EXISTS (
