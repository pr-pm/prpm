...
4. **Install with custom name:**
   ```bash
   prpm install typescript-rules --as different-name
   ```
...
prpm install typescript-rules --verbose
```
...
curl -I https://registry.prpm.dev/packages/typescript-rules
```
