...
**Problem:** After running `npm install -g @pr-pm/cli`, the `prpm` command is not found.
...
3. **Use npx as alternative:**
   ```bash
   npx @pr-pm/cli install author/package
   ```

4. **Reinstall with correct permissions:**
   ```bash
   # On Linux/Mac, may need sudo
   sudo npm install -g @pr-pm/cli
   ```
...
**Common causes:**
1. **Incorrect package name:** Packages must include author namespace
   ```bash
   # ✗ Wrong
   prpm install typescript-rules

   # ✓ Correct
   prpm install author/typescript-rules
   ```
...
2. **Private package without authentication:**
   ...
   # Login first
   prpm login
   prpm install author/private-package
   ```
...
2. **Uninstall conflicting package first:**
   ...
   prpm install author/new-package
   ```
...
4. **Install with custom name:**
   ```bash
   prpm install author/package --as different-name
   ```
...
prpm install author/package --verbose
```
...
curl -I https://registry.prpm.dev/packages/author/package-name
```
