...
prpm install \
  author/react-rules \
  author/typescript-config \
  author/testing-guidelines
```
...
```bash
prpm install author/very-long-package-name --as short-name
```
...
2. **Follow the interactive prompts:**
   - Package name: `my-username/my-cursor-rules`
...
4. **Test locally:**
   ...
   cd ../test-project
   prpm install /path/to/my-cursor-rules
   ```
...
5. **Verify publication:**
   ```bash
   prpm info my-username/my-cursor-rules
   ```
...
1. **Update version in `prpm.json`:**
   ```json
   {
     "name": "my-username/my-cursor-rules",
     "version": "1.1.0",
     ...
   }
   ```
...
prpm install \
  author/typescript-rules \
  author/react-best-practices \
  author/testing-guidelines \
  author/git-commit-standards
...
### Global Packages (Future Feature)

While not yet available, global packages will allow:
```bash
# Install globally (coming soon)
prpm install --global author/package

# Will be available in all projects
```
...
steps:
  - name: Install PRPM packages
    run: |
      npm install -g prpm
      prpm install author/package1 author/package2
    env:
...
prpm install author/package --version 1.0.0
```
...
# Force reinstall
prpm uninstall package-name
prpm install author/package-name --force
...
3. **Use meaningful custom names:**
   ```bash
   prpm install author/long-package-name --as descriptive-name
   ```
...
4. **Document your dependencies:**
   ...
   This project uses:
   - `author/typescript-rules` - TypeScript coding standards
   - `author/react-patterns` - React best practices
   ```
