...
```bash
prpm install very-long-package-name --as short-name
```
...
prpm install --global typescript-rules
...
steps:
  - name: Install PRPM packages
    run: |
      npm install -g prpm
      prpm install typescript-rules1 typescript-rules2
    env:
...
prpm install typescript-rules --version 1.0.0
```
...
3. **Use meaningful custom names:**
   ```bash
   prpm install long-package-name --as descriptive-name
   ```
