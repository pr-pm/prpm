...
<Info>
  Packages are namespaced by author. The format is `typescript-rules`.
</Info>
...
- Package name (format: `typescript-rules`)
- Description
...
<Success>
  Your package is now live on the PRPM registry! Others can find it with `prpm search` and install it with `prpm install typescript-rules`.
</Success>
