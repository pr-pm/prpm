...
# Prerequisites:
#   - Registry running at $PRPM_REGISTRY (default: http://localhost:3111)
...
#   PRPM_REGISTRY - Registry URL (default: http://localhost:3111)
#   DEBUG - Enable verbose output (default: false)

set -e

# Configuration
REGISTRY_URL="${PRPM_REGISTRY:-http://localhost:3111}"
...
for batch_dir in "$TEST_DIR"/batch-*; do
  if [ -d "$batch_dir" ] && [ -f "$batch_dir/prpm.json" ]; then
    ...
    cd "$batch_dir"
    if prpm publish --registry "$REGISTRY_URL" 2>&1; then
...
VERIFY_TEST="$PROJECT_ROOT/$TEST_DIR/verify-installs.test.ts"
if [ -f "$VERIFY_TEST" ]; then
  ...
  if prpm install @ci-test/cursor-rule --as cursor --registry "$REGISTRY_URL" 2>&1; then
...
if prpm install collections/ci-test-collection --registry "$REGISTRY_URL" 2>&1; then
